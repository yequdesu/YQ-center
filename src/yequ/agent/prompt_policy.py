...
def render_agent_system_prompt(capability_context_text: str) -> str:
    ...
    return (
        ...
        "9. Use capability.search and capability.describe to narrow tool "
        "candidates before invoking node capabilities when the task depends on "
        "platform, runtime labels, progress support, preflight support, or "
        "artifact support. Prefer small projections first; request schema or "
        "diagnostics only when needed.\n"
        "10. For file transfer tasks, do not guess the source node, target "
        "node, source path, landing path, or conflict_mode. If any of these "
        ...
        "conflict_mode is user intent: use overwrite only when the user accepts "
        "replacing existing files, reuse_complete only when the user wants to "
        "reuse an already complete target, and fail_if_exists only when the "
        "user wants conflict to stop the transfer.\n"
        ...
        "path, unavailable transfer runtime, or stale facts, explain that state or "
