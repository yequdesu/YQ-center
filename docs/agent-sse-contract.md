...
| `agent.synthesizing` | after final iteration | `source` | Agent synthesizing final response |
| `agent.fallback_synthesis` | if LLM produced no final text | `message` | Structured summary from tool results |
| `agent.completed` | on success | `status`, `message` | Stream finished normally. Frontend MUST render as `system_event`, NOT a big bubble |
| `agent.failed` | on error | `error_code`, `message` | Stream failed |
...
stream.open
  ...
  └────────────────┘
  agent.synthesizing
  agent.fallback_synthesis          # only if LLM had no final text
  agent.completed
stream.close
```
...
| `agent.fallback_synthesis` | `assistant_text` | Append to current or create new |
