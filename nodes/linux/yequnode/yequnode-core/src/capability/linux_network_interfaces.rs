...
impl Capability for LinuxNetworkInterfaces {
    fn manifest() -> CapabilityManifest {
        CapabilityManifest {
            ...
            description: "List network interfaces with IP addresses, MAC, and traffic counters."
                .into(),
            agent_description: Some(
                "List network interfaces with IP addresses, MAC, and traffic counters.".into(),
            ),
            input_schema: json!({
                "type": "object",
                "properties": {},
...
fn parse_json_interfaces(data: &Value) -> Vec<Value> {
    ...
    arr.iter()
        .map(|iface| {
            ...
            let rx_bytes = stats64
                .and_then(|s| {
                    s.get("rx")
                        .and_then(|r| r.get("bytes").and_then(|v| v.as_u64()))
                })
                .unwrap_or(0);
            let rx_packets = stats64
                .and_then(|s| {
                    s.get("rx")
                        .and_then(|r| r.get("packets").and_then(|v| v.as_u64()))
                })
                .unwrap_or(0);
            let tx_bytes = stats64
                .and_then(|s| {
                    s.get("tx")
                        .and_then(|t| t.get("bytes").and_then(|v| v.as_u64()))
                })
                .unwrap_or(0);
            let tx_packets = stats64
                .and_then(|s| {
                    s.get("tx")
                        .and_then(|t| t.get("packets").and_then(|v| v.as_u64()))
                })
                .unwrap_or(0);
