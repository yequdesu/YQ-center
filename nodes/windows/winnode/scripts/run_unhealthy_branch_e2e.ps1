...
if (-not (Test-IsAdmin)) {
    ...
    Write-Host "  cd `"$Root`""
    Write-Host "  powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run_unhealthy_branch_e2e.ps1"
    exit 5
}

New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$logPath = Join-Path $LogDir "unhealthy-branch-$timestamp.json"
...
try {
    ...
    $timeline = Invoke-Json `
        -Uri "$BaseUrl/admin/timeline?approval_id=$approvalId&limit=100" `
        -Headers $adminHeaders `
        -TimeoutSec 30
    ...
    if ($timeline.PSObject.Properties.Name -contains "value") {
        $eventTypes = @($timeline.value | ForEach-Object { $_.event_type })
    } elseif ($timeline -is [System.Array]) {
        $eventTypes = @($timeline | ForEach-Object { $_.event_type })
    }
    ...
    $result = [ordered]@{
        ok = (
            ...
            $afterService.Status -eq "Running"
        )
    ...
    if ($result.ok) {
        Write-Host "[DONE] Unhealthy branch E2E passed." -ForegroundColor Green
        exit 0
    }

    Write-Host "[FAIL] Unhealthy branch E2E did not match expectations." -ForegroundColor Red
    exit 2
} finally {
