...
@pytest.mark.asyncio
@pytest.mark.parametrize("prompt,expected_function,expected_input", [
...
async def test_deepseek_tool_selection(prompt, expected_function, expected_input):
    ...
    assert expected_function in names, (
        f"Expected {expected_function} for '{prompt}', got {names}"
    )
    ...
    if expected_input:
        ...
        for key, val in expected_input.items():
            assert matching[0]["input"].get(key) == val, (
                f"Expected input {key}={val} for '{prompt}', "
                f"got {matching[0]['input']}"
            )
