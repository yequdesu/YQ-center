...
async def plan_setup(client: AsyncClient):
    ...
    await client.post("/admin/nodes", json={
        "node_id": node_id, "node_name": f"IR-{node_id}", "token": token,
    })
    await client.post("/yqp/", json=make_yqp_envelope(
        "node.hello", node_id, {"daemon_version": "0.1.0"},
    ), headers=auth)
    funcs = [
    ...
    await client.post("/yqp/", json=make_yqp_envelope(
        ...
        payload={"plugins": [{"plugin_id": "test", "plugin_version": "1.0", "functions": funcs, "signals": []}]},
    ), headers=auth)
...
async def test_readonly_check_returns_ready(client: AsyncClient, plan_setup):
    ...
    assert r.status_code == 201 or r.status_code == 200, f"Status: {r.status_code}, body: {r.text[:500]}"
    data = r.json()
